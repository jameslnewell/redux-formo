    ...
    class DecoratedForm extends React.Component {
        ...
        this.handlers = Object.assign.apply(null, fieldNames.map(fieldName => {
          return {[fieldName]: {
            ...
            onBlur: () => {
              ...
              let fieldValues = getValues(this.props.fields, initialFieldValues);

              if (filterOnBlur) {
                props.actions.filter(filter, fieldName, fieldValues);
              }

              if (validateOnBlur) {
                props.actions.validate(validate, fieldName, fieldValues);
              }
      ...
      handleSubmit(event) {
        ...
        let
          props = this.props,
          valid = true,
          fieldValues = getValues(props.fields, initialFieldValues);
        ;
        ...
        fieldNames.forEach(fieldName => {
          ...
          if (filterOnSubmit)  {
            fieldValues[fieldName] = props.actions.filter(filter, fieldName, fieldValues);
          }

          if (validateOnSubmit) {
            valid = props.actions.validate(validate, fieldName, fieldValues) && valid;
          }
        ...
        if (valid && submit) {
          props.actions.submit(submit, fieldValues);
        }
